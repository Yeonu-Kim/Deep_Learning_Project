...
class SGG(pl.LightningModule):
    def __init__(
        ...
        # ADD: Graph Only 모드 추가
        print(f"[INFO] Graph-only mode: {self.graph_only}")
    ...
    def common_step(self, batch, batch_idx):
        ...
        if self.graph_only:
            # Relation loss만 사용
            loss = torch.tensor(0.0, device=outputs.loss.device, dtype=outputs.loss.dtype)
            
            for key, value in loss_dict.items():
                # loss_rel, connectivity_loss만 포함
                if 'loss_rel' in key or 'connectivity' in key:
                    loss += value
        else:
            # 기존 방식: 모든 loss 사용
            loss = outputs.loss
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.graph_only:
            log_dict.update({
                f"training_{k}": v.item() 
                for k, v in loss_dict.items() 
                if 'loss_rel' in k or 'connectivity' in k
            })
        else:
...
if __name__ == "__main__":
    ...
    # Path
    parser.add_argument("--data_path", type=str, default="dataset/crohme")
    parser.add_argument(
        "--output_path",
        type=str,
        required=True,
